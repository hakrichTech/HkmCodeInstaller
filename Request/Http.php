<?php
namespace Hkm_Request;

use Hkm_Request\Requests;
use Hkm_code\Cookie\Cookie;
use Hkm_Request\Requests\Requests_Cookie;
use Hkm_Request\Requests\Requests_Exception;
use Hkm_Request\Requests\Proxy\Requests_Proxy_HTTP;
use Hkm_Request\Requests\Cookie\Requests_Cookie_Jar;
...
class Http {
	 ...
	 *
	 * }
	 * @return array|Object Array containing 'headers', 'body', 'response', 'cookies', 'filename'.
	 *                        A Object instance upon error.
	 */
	public function request( $url, $args = array() ) {
		$defaults = array(
			...
			'redirection'         => 5,
			'httpversion'         => '1.0',
			'user-agent'          => 'Hkm_Code/2.0.1; https://hkmcode.hakrichteam.com',
		...
		$parsed_args = hkm_parse_args( $args, $defaults );

		// $parsed_args = hkm_apply_runtime_filters( 'http_request_args', $parsed_args, $url );
		 ...
		 * @since 2.9.0
		 *
		 * @param false|array|Object $preempt     A preemptive return value of an HTTP request. Default false.
		 * @param array                $parsed_args HTTP request arguments.
		 * @param string               $url         The request URL.
		 */
		$pre = hkm_apply_runtime_filters( 'pre_http_request', false, $parsed_args, $url );

		if ( false !== $pre ) {
			return $pre;
		}

		if ( function_exists( 'hkm_kses_bad_protocol' ) ) {
			if ( $parsed_args['reject_unsafe_urls'] ) {
                hkm_helper('url');
				$url = hkm_http_validate_url( $url );
			}
			if ( $url ) {
				$url = hkm_kses_bad_protocol( $url, array( 'http', 'https', 'ssl' ) );
			}
		}
		...
		if ( $parsed_args['stream'] ) {
			if ( empty( $parsed_args['filename'] ) ) {
				$parsed_args['filename'] = WRITEPATH ."temp/". basename( $url );
			}
			...
			if ( ! hkm_is_writable( dirname( $parsed_args['filename'] ) ) ) {
				return [
                    'error' => true,
		...
		if ( ! is_array( $parsed_args['headers'] ) ) {
			$processedHeaders       = Http::processHeaders( $parsed_args['headers'] );
		...
		$options = array(
			...
			'hooks'     => new HTTP_Requests_Hooks( $url, $parsed_args ),
		);
		...
		if ( ! empty( $parsed_args['cookies'] ) ) {
			$options['cookies'] = Http::normalize_cookies( $parsed_args['cookies'] );
		}
		 ...
		 * @param string $url        The request URL.
		 */
		$options['verify'] = hkm_apply_runtime_filters( 'https_ssl_verify', $options['verify'], $url );
		 ...
		 */
        return $response;
		// return hkm_apply_runtime_filters( 'http_response', $response, $parsed_args, $url );
	}
	...
	public static function validate_redirects( $location ) {
        hkm_helper('url');
		if ( ! hkm_http_validate_url( $location ) ) {
			throw new Requests_Exception( 'A valid URL was not provided.', 'hkm_http.redirect_failed_validation' );
		}
	}
	...
	public function _get_first_available_transport( $args, $url = null ) {
		 ...
		 */
		$request_order = hkm_apply_runtime_filters( 'http_api_transports', $transports, $args, $url );
	...
	private function _dispatch_request( $url, $args ) {
		...
		if ( ! $class ) {
			...
			// return new Object( 'http_failure', __( 'There are no HTTP transports available which can complete the requested request.' ) );
		}
		...
		if ( is_hkm_error( $response ) ) {
			return $response;
		}

		/** This filter is documented in wp-includes/class-http.php */
        return $response;
		// return hkm_apply_runtime_filters( 'http_response', $response, $args, $url );
	}
	 ...
	 * `HKM_ACCESSIBLE_HOSTS` will allow additional hosts to go through for requests. The format of the
	 * `HKM_ACCESSIBLE_HOSTS` constant is a comma separated list of hostnames to allow, wildcard domains
	 * are supported, eg `*.wordpress.org` will allow for all subdomains of `wordpress.org` to be contacted.
	 *
	 * @since 2.8.0
	 *
	 * @link https://core.trac.wordpress.org/ticket/8927 Allow preventing external requests.
	 * @link https://core.trac.wordpress.org/ticket/14636 Allow wildcard domains in HKM_ACCESSIBLE_HOSTS
	 *
	...
	public function block_request( $uri ) {
		...
		if ( ! defined( 'HKM_HTTP_BLOCK_EXTERNAL' ) || ! HKM_HTTP_BLOCK_EXTERNAL ) {
			return false;
		}
		...
		$home = parse_url( hkm_config('App')::$baseUrl );
		...
		if ( 'localhost' === $check['host'] || ( isset( $home['host'] ) && $home['host'] == $check['host'] ) ) {
			 ...
			 */
			return hkm_apply_runtime_filters( 'block_local_requests', false );
		}

		if ( ! defined( 'HKM_ACCESSIBLE_HOSTS' ) ) {
			return true;
		}
		...
		if ( null === $accessible_hosts ) {
			$accessible_hosts = preg_split( '|,\s*|', HKM_ACCESSIBLE_HOSTS );

			if ( false !== strpos( HKM_ACCESSIBLE_HOSTS, '*' ) ) {
	 ...
	 *
	 * @deprecated 4.4.0 Use hkm_parse_url()
	 * @see hkm_parse_url()
	 *
	...
	public static function make_absolute_url( $maybe_relative_path, $url ) {
		...
		$url_parts = hkm_parse_url( $url );
		if ( ! $url_parts ) {
			return $maybe_relative_path;
		}

		$relative_url_parts = hkm_parse_url( $maybe_relative_path );
	 ...
	 * @return array|false|Object An HTTP API response array if the redirect is successfully followed,
	 *                              false if no redirect is present, or a Object object if there's an error.
	 */
	public static function handle_redirects( $url, $args, $response ) {
		...
		if ( $args['redirection']-- <= 0 ) {
			return [
                'error' => true,
                'message' => 'http_request_failed: Too many redirects.'
            ];
		}
		...
		return hkm_remote_request( $redirect_location, $args );
	}
