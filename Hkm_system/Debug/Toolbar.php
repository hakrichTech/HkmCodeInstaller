...
class Toolbar
{
		...
		foreach ($config::$collectors as $collector)
		{
			if (! class_exists($collector))
			{
				log_message('critical', 'Toolbar collector does not exists(' . $collector . ').' .
						'please check $collectors in the '.APP_CONFIG_NAMESPACE.'\Toolbar.php file.');
				continue;
			}
	...
	public static function RUN(float $startTime, float $totalTime, Request $request, Response $response): string
	{
		...
		if (! empty($_SESSION))
		{
			foreach ($_SESSION as $key => $value)
			{
				...
				$data['vars']['session'][esc($key)] = is_string($value) ? hkm_esc($value) : '<pre>' . hkm_esc(print_r($value, true)) . '</pre>';
			}
		}

		foreach ($request::GET_GET() as $name => $value)
		{
			$data['vars']['get'][esc($name)] = is_array($value) ? '<pre>' . hkm_esc(print_r($value, true)) . '</pre>' : hkm_esc($value);
		}

		foreach ($request::GET_POST() as $name => $value)
		{
			$data['vars']['post'][esc($name)] = is_array($value) ? '<pre>' . hkm_esc(print_r($value, true)) . '</pre>' : hkm_esc($value);
		}
		...
		$data['vars']['headers'][esc($request::HEADERS()['Host'])] = hkm_esc(implode(', ', $options));


		foreach ($request::GET_COOKIE() as $name => $value)
		{
			$data['vars']['cookies'][esc($name)] = hkm_esc($value);
		}
	...
	public static function PREPARE(Request $request = null, Response $response = null)
	{
		...
		if (HKM_DEBUG && ! hkm_is_cli())
		{
			...
			$toolbar = Services::TOOLBAR(hkm_config('Toolbar'));
			...
			if ($request::IS_AJAX() || strpos($format, 'html') === false)
			{
				$response::SET_HEADER('Debugbar-Time', "$time")
						::SET_HEADER('Debugbar-Link', site_url("?debugbar_time={$time}"))
						::GET_BODY();
	...
	protected static function FORMAT(string $data, string $format = 'html'): string
	{
		...
		if (self::$config::$maxHistory !== 0)
		{
			$history = new History();
			$history::SET_FILES(
				(int) Services::REQUEST()::GET_GET('debugbar_time'),
		...
		switch ($format)
		{
			case 'html':
				$data['styles'] = [];
				extract($data);
				$parser = Services::PARSE(self::$config::$viewsPath, null, false);
				ob_start();
