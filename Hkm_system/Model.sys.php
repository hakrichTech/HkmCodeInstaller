...
class Model extends BaseModel
{
	...
	public function __construct(ConnectionInterface &$db = null, ValidationInterface $validation = null)
	{
		 ...
		 */
		$db = $db ?? hkm_config('Database')::CONNECT(self::$DBGroup);
	...
	{
		$builder = self::BUILDER();
		return $builder->where($key, $value,$escape)->get()->getResult(self::$thiss::$tempReturnType);
	...
	protected static function DO_INSERT(array $data)
	{
		...
		if ($result)
		{
			self::$insertID = ! self::$thiss::$useAutoIncrement ? $data[self::$thiss::$primaryKey] : self::$db::INSERT_ID();
		}
	...
	protected static function DO_INSERT_BATCH(?array $set = null, ?bool $escape = null, int $batchSize = 100, bool $testing = false)
	{
		...
		return self::BUILDER()->testMode($testing)->insertBatch($set, $escape, $batchSize);
	}
	...
	{
		return self::BUILDER()->testMode($returnSQL)->updateBatch($set, $index, $batchSize);
	}
	...
	{
		self::BUILDER()->where(self::$thiss::$table . '.' . self::$thiss::$deletedField . ' IS NOT NULL');
	}
	...
	{
		return self::BUILDER()->testMode($returnSQL)->replace($data);
	}
	...
	protected static function DO_ERRORS()
	{
		...
		return [get_class(self::$db) => $error['message']];
	}
	 ...
	 * Works with self::$builder to get the Compiled select to
	...
	{
		$total  = self::BUILDER()->countAllResults(false);
		$offset = 0;
	...
	public static function COUNT_ALL_RESULTS(bool $reset = true, bool $test = false)
	{
		if (self::$tempUseSoftDeletes)
		{
			self::BUILDER()->where(self::$thiss::$table . '.' . self::$thiss::$deletedField, '');
		}
		...
		return self::BUILDER()->testMode($test)->countAllResults($reset);
	}
	...
	public static function BUILDER(?string $table = null)
	{
		...
		if (self::$builder instanceof BaseBuilder)
		{
			// Make sure the requested table matches the builder
			if ($table && self::$builder->getTable() !== $table)
			{
		...
		if (! self::$db instanceof BaseConnection)
		{
			self::$db = hkm_config('Database')::CONNECT(self::$DBGroup);
		}
	...
	public static function checkCache(array $data)
	{
		if (isset($data['id']) ) {
			...
			if (isset(self::$thiss::$cachedResponse[$data['id']])&&$item = self::GET_CACHED_ITEM($data['id'])) {
				$data['data']       = $item;
	...
	{
		if (isset($data['id'])) {
			self::$thiss::$cachedResponse[$data['id']] = $data['data'];
		}
        return $data;
	}
	...
	{
		return self::$thiss::$cachedResponse[$id];
	}
}
