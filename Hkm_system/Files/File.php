...
class File extends SplFileInfo
{
	...
	{
		return self::$size ?? (self::$size = parent::getSize());
	}
	...
	{
		$mime = APP_CONFIG_NAMESPACE."\Mimes";
		$mimes = new $mime();
	...
	public static function GET_MIME_TYPE(): string
	{
		...
		$mimeType = finfo_file($finfo, parent::getRealPath() ?: parent::__toString());
		finfo_close($finfo);
		return $mimeType;
	}
	...
	public static function MOVE(string $targetPath, string $name = null, bool $overwrite = false)
	{
		...
		$name        = $name ?? parent::getBaseName();
		$destination = $overwrite ? $targetPath . $name : self::GET_DESTINATION($targetPath . $name);

		$oldName = parent::getRealPath() ?: parent::__toString();

		if (! @rename($oldName, $destination))
		{
			$error = error_get_last();
			throw FileException::FOR_UNABLE_TO_MOVE(parent::getBaseName(), $targetPath, strip_tags($error['message']));
		}
