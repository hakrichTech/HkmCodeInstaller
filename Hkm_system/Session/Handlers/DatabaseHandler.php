...
namespace Hkm_code\Session\Handlers;

use Hkm_code\Database\BaseConnection;
use Hkm_code\Session\Exceptions\SessionException;
...
class DatabaseHandler extends BaseHandler
{
	...
	public function __construct( $config, string $ipAddress)
	{
		...
		$this->DBGroup = $config::$sessionDBGroup ?? hkm_config(Database::class)->defaultGroup;

		$this->db = hkm_config('Database')::connect($this->DBGroup);
