...
/**
 * Implementation of Hkm_code session container.
 *
...
class Session implements SessionInterface
{
	 ...
	 * @var string
	 */
	protected $sessionCookieName = 'hkm_session';
	...
	public function start()
	{
		...
		{
			if (! isset($_SESSION['__hkm_last_regenerate']))
			{
				$_SESSION['__hkm_last_regenerate'] = time();
			}
			elseif ($_SESSION['__hkm_last_regenerate'] < (time() - $regenerateTime))
			{
	...
	protected function initVars()
	{
		...
		foreach ($_SESSION['__hkm_vars'] as $key => &$value)
		{
			if ($value === 'new')
			{
				$_SESSION['__hkm_vars'][$key] = 'old';
			}
			// DO NOT move this above the 'new' check!
			elseif ($value === 'old' || $value < $currentTime)
			{
				unset($_SESSION[$key], $_SESSION['__hkm_vars'][$key]);
			}
		}

		if (empty($_SESSION['__hkm_vars']))
		{
			unset($_SESSION['__hkm_vars']);
		}
	}
	...
	public function get(string $key = null)
	{
		...
		$userdata = [];
		$_exclude = array_merge(['__hkm_vars'], $this->getFlashKeys(), $this->getTempKeys());
	...
	public function getFlashdata(string $key = null)
	{
		if (isset($key))
		{
			return (isset($_SESSION['__hkm_vars'], $_SESSION['__hkm_vars'][$key], $_SESSION[$key]) &&
					! is_int($_SESSION['__hkm_vars'][$key])) ? $_SESSION[$key] : null;
		}
	...
	public function markAsFlashdata($key): bool
	{
		if (is_array($key))
		{
			...
			$_SESSION['__hkm_vars'] = isset($_SESSION['__hkm_vars']) ? array_merge($_SESSION['__hkm_vars'], $new) : $new;
	...
	public function unmarkFlashdata($key)
	{
		...
		foreach ($key as $k)
		{
			if (isset($_SESSION['__hkm_vars'][$k]) && ! is_int($_SESSION['__hkm_vars'][$k]))
			{
				unset($_SESSION['__hkm_vars'][$k]);
			}
		}
	...
	{
		if (! isset($_SESSION['__hkm_vars']))
		{
			return [];
		}

		$keys = [];
		foreach (array_keys($_SESSION['__hkm_vars']) as $key)
		{
			if (! is_int($_SESSION['__hkm_vars'][$key]))
			{
				$keys[] = $key;
			}
		}
	...
	public function markAsTempdata($key, int $ttl = 300): bool
	{
		...
		if (is_array($key))
		{
			...
			$_SESSION['__hkm_vars'] = isset($_SESSION['__hkm_vars']) ? array_merge($_SESSION['__hkm_vars'], $temp) : $temp;
