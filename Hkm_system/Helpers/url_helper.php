...
if (! function_exists('hkm_current_url'))
{
	 ...
	 * @return string|URI
	 */
	function hkm_current_url(bool $returnObject = false, IncomingRequest $request = null)
	{
...
if (! function_exists('hkm_previous_url'))
{
	...
	function hkm_previous_url(bool $returnObject = false)
	{
		...
		$referer = $_SESSION['_hkm_previous_url'] ?? Services::REQUEST()::getServer('HTTP_REFERER', FILTER_SANITIZE_URL);
...
if (! function_exists('hkm_uri_string'))
{
	...
	{
		return $relative
			? ltrim(Services::REQUEST()::GET_PATH(), '/')
			: Services::REQUEST()::GET_URI()::GET_PATH();
	}
}
