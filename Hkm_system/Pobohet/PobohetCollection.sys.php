...
class PobohetCollection 
{
	...
	{
		$data = Services::DOM_XML_IN_ARRAY();
		$data::XML_READER('Pobohet/Pobohet2','Pobohet','Default');
		...
		$data::XML_READER('Pobohet/Pobohet2','Pobohet','All');
		$routes = $data::$fileData;
		unset($routes['Pobohet']);
		$data::XML_READER('Pobohet/Pobohet2','Pobohet',str_replace("HakrichApp",'',APP_NAME));
		...
		$data::XML_READER('Pobohet/Pobohet2','Pobohet',str_replace("HakrichApp",'',APP_NAME).'HakrichApp');
	...
	protected static function INITIALIZE_POBOHET()
	{
		...
		foreach(self::$data as $pobohet => $value){
			if (is_array($value['pobohet'])) foreach ($value['pobohet'] as $pobohet)self::CREATE($pobohet,$value['url'],$value['controller']."::".$value['method'],$value['options']??[]);
			else self::CREATE($value['pobohet'],$value['url'],$value['controller']."::".$value['method'],$value['options']??[]);
		}
	...
	{
		$D =  Services::DOM_XML_IN_ARRAY()::XML_ADD_DATA("Pobohet/Pobohet2",$new_route,'Pobohet','url');
		self::INITIALIZE();
		return $D;
	}
	...
	public static function MATCH(array $verbs = [], string $from = '', $to = '', array $options = null)
	{
	...
	{
		return strtr($route, ['{locale}' => Services::REQUEST()::GET_LOCAL()]);
	}
