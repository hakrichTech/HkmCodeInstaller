...
namespace Hkm_code\Pager;

use Hkm_code\Exceptions\HTTP\HTTPException;
use Hkm_code\HTTP\URI;
use Hkm_code\Pager\Exceptions\PagerException;
...
class Pager implements PagerInterface
{
	...
	protected function displayLinks(string $group, string $template): string
	{
		...
		return $this->view::SET_VAR('pager', $pager)
	...
	public function getPageURI(int $page = null, string $group = 'default', bool $returnObject = false)
	{
		...
		return $returnObject === true ? $uri : URI::CREATE_URI_STRING($uri::GET_SCHEME(), $uri::GET_AUTHORITY(), $uri::GET_PATH(), $uri::GET_QUERY(), $uri::GET_FRAGMENT());
	}
	...
	protected function ensureGroup(string $group, int $perPage = null)
	{
		...
		$this->groups[$group] = [
			'uri'          => clone hkm_current_url(true),
			'hasMore'      => false,
			'total'        => null,
			'perPage'      => $perPage ?? $this->config->perPage,
			'pageCount'    => 1,
