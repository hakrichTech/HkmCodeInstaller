...
class Application
{
	...
	protected static function GET_REQUEST_OBJECT()
	{
		...
		if (hkm_is_cli() && ENVIRONMENT !== 'testing') {
			self::$request = Services::CLI_REQUEST(self::$config);
		} else {

			self::$request = Services::REQUEST(self::$config);
	...
	public static function TRY_ROUTE_IT(PobohetCollection $routes = null)
	{
		if ($routes === null) {
			...
			$routes::GET('/', 'Home::index', ['namespace' => APP_NAMESPACE . "\Controllers", 'filter' => 'login']);
			$routes::GET('/login', 'Home::login', ['namespace' => APP_NAMESPACE . "\Controllers"]);
			$routes::POST('/login', 'Home::post', ['namespace' => APP_NAMESPACE . "\Controllers"]);
		}



		self::$router = Services::ROUTER($routes, self::$request);
	...
	{
		self::$response = Services::RESPONSE(self::$config);
	...
	public static function CREATE_CONTROLLER()
	{
		...
		$class::INIT_CONTROLLER(self::$request, self::$response, Services::LOGGER());
	...
	public static function STORE_PREVIOUS_URL($uri)
	{
		...
		if (isset($_SESSION)) {
			$_SESSION['_hkm_previous_url'] = URI::CREATE_URI_STRING($uri::GET_SCHEME(), $uri::GET_AUTHORITY(), $uri::GET_PATH(), $uri::GET_QUERY(), $uri::GET_FRAGMENT());
		}
	}
