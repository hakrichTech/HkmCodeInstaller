...
final class Environment extends BaseCommand
{
	...
	public function run(array $params)
	{
		if ($params === [])
		{
			CLI::write(sprintf('Your environment is currently set as %s.', CLI::color($_SERVER['HKM_ENVIRONMENT'] ?? ENVIRONMENT, 'green')));
			CLI::newLine();
		...
		putenv('HKM_ENVIRONMENT');
		unset($_ENV['HKM_ENVIRONMENT'], $_SERVER['HKM_ENVIRONMENT']);
	...
	private function writeNewEnvironmentToEnvFile(string $newEnv): bool
	{
		...
		$pattern = preg_quote($_SERVER['HKM_ENVIRONMENT'] ?? ENVIRONMENT, '/');
		$pattern = sprintf('/^[#\s]*HKM_ENVIRONMENT[=\s]+%s$/m', $pattern);

		return file_put_contents(
			$envFile,
			preg_replace($pattern, "\nHKM_ENVIRONMENT = {$newEnv}", file_get_contents($envFile), -1, $count)
		) !== false && $count > 0;
	}
}
