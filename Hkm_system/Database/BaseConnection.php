...
abstract class BaseConnection implements ConnectionInterface
{
	...
	{
		return empty(self::$database) ? '' : self::$database;
	}
	...
	public static function QUERY(string $sql, $binds = null, bool $setEscapeFlags = true, string $queryClass = '')
	{
		...
		if (! empty(self::$swapPre) && ! empty(self::$DBPrefix))
		{
			$query::SWAP_PREFIX(self::$DBPrefix, self::$swapPre);
		}
	...
	public static function PROTECT_INDENTIFIERS($item, bool $prefixSingle = false, bool $protectIdentifiers = null, bool $fieldExists = true)
	{
		...
		if (strpos($item, '.') !== false)
		{
			...
			if (self::$DBPrefix !== '')
			{
				...
				{
					$parts[$i] = preg_replace('/^' . self::$swapPre . '(\S+?)/', self::$DBPrefix . '\\1', $parts[$i]);
				}
				// We only add the table prefix if it does not already exist
				elseif (strpos($parts[$i], self::$DBPrefix) !== 0)
				{
		...
		if (self::$DBPrefix !== '')
		{
			...
			{
				$item = preg_replace('/^' . self::$swapPre . '(\S+?)/', self::$DBPrefix . '\\1', $item);
			}
			// Do we prefix an item with no segments?
			elseif ($prefixSingle === true && strpos($item, self::$DBPrefix) !== 0)
			{
