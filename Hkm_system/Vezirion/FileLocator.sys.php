...
class FileLocator
{
	...
	public static function LOCATE_FILE(string $fileOrginal, string $folder = null, string $ext = 'php') 
	{
		...
		{
			return static::LEGACY_LOCATE($file, $folder);
		}
	...
	{
		$path1 = static::ENSURE_EXT($path, $ext);
		...
		{   
			...
			{
				$file = str_replace("\\","/",$namespace['path'] . ltrim($path1,$namespace['prefix']));
				if(is_file($file)){
					$fullPath = $file;
	...
	protected static function GET_NAMESPACES(string $prefix = null)
	{
		...
		$ns = HKM_NAMESPACES__;
		if (is_null($prefix)) {
			if (APP_NAMESPACE!="App") unset($ns['system']['App\\']);
			foreach ($ns['system'] as $prefix => $paths) foreach ($paths as $path) $system[] = ['prefix' => $prefix,'path'   => rtrim($path, '\\/') . DIRECTORY_SEPARATOR,];
			foreach ($ns['app'] as $prefix => $paths)  foreach ($paths as $path)$namespaces[] = ['prefix' => $prefix,'path'   => rtrim($path, '\\/') . DIRECTORY_SEPARATOR,];
			...
			if (isset($ns['system'][$prefix])) return $ns['system'][$prefix];
			if (isset($ns['app'][$prefix])) return $ns['app'][$prefix];
