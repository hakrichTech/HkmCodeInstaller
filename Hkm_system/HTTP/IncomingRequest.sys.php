...
class IncomingRequest extends Request
{
	...
	public static function NEGOTIATE(string $type, array $supported, bool $strictMatch = false): string
	{
		...
		{
			self::$negotiator = Services::NEGOTIATOR(self::$thiss, true);
		}
	...
	public static function GET_JSON_VAR(string $index, bool $assoc = false, ?int $filter = null, $flags = null)
	{
		...
		$data = dot_array_search($index, self::GET_JSON(true));
	...
	public static function GET_OLD_INPUT(string $key)
	{
		...
		if (empty($_SESSION['_hkm_old_input']))
		{
			return null;
		}

		// Check for the value in the POST array first.
		if (isset($_SESSION['_hkm_old_input']['post'][$key]))
		{
			return $_SESSION['_hkm_old_input']['post'][$key];
		}

		// Next check in the GET array.
		if (isset($_SESSION['_hkm_old_input']['get'][$key]))
		{
			return $_SESSION['_hkm_old_input']['get'][$key];
		}

		helper('array');

		// Check for an array value in POST.
		if (isset($_SESSION['_hkm_old_input']['post']))
		{
			$value = dot_array_search($key, $_SESSION['_hkm_old_input']['post']);
			if (! is_null($value))
			{
				return $value;
			}
		...
		if (isset($_SESSION['_hkm_old_input']['get']))
		{
			$value = dot_array_search($key, $_SESSION['_hkm_old_input']['get']);
			if (! is_null($value))
			{
				return $value;
			}
