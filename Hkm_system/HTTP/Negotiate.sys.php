...
class Negotiate
{
	...
	{
		return self::GET_BEST_MATCH($supported, self::$request::GET_HEADER_LINE('accept'), true, $strictMatch);
	}
	...
	{
		$match = self::GET_BEST_MATCH($supported, self::$request::GET_HEADER_LINE('accept-charset'), false, true);
	...
	public static function ENCODING(array $supported = []): string
	{
		...
		return self::GET_BEST_MATCH($supported, self::$request::GET_HEADER_LINE('accept-encoding'));
	}
	...
	{
		return self::GET_BEST_MATCH($supported, self::$request::GET_HEADER_LINE('accept-language'), false, false, true);
	}
	 ...
	 */
	protected static function GET_BEST_MATCH(array $supported, string $header = null, bool $enforceTypes = false, bool $strictMatch = false, bool $matchLocales = false): string
	{
		if (empty($supported))
		{
		...
		foreach ($acceptable as $accept)
		{
			...
			{
				if (self::MATCH($accept, $available, $enforceTypes, $matchLocales))
				{
					return $available;
				}
			}
	 ...
	 * @return boolean
	 */
	protected static function MATCH(array $acceptable, string $supported, bool $enforceTypes = false, $matchLocales = false): bool
	{
