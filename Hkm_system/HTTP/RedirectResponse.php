...
class RedirectResponse extends Response
{
	...
	{
		$route = Services::ROUTES()::RESERVE_ROUTE($route, ...$params);
		...
		return self::REDIRECT(site_url($route), $method, $code);
	}
	...
	public static function BACK(int $code = null, string $method = 'auto')
	{
		...
		return self::REDIRECT(previous_url(), $method, $code);
	}
	...
	public static function WITH_INPUT()
	{
		...
		$session->setFlashdata('_hkm_old_input', [
			'get'  => $_GET ?? [],
			'post' => $_POST ?? [],
		]);
		...
		{
			$session->setFlashdata('_hkm_validation_errors', serialize($validation->getErrors()));
		}
	...
	{
		self::$cookieStore = new CookieStore(Services::RESPONSE()::GET_COOKIES());
