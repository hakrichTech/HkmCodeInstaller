...
class MigrationGenerator extends BaseCommand
{
	...
	public function run(array $params)
	{
		...
		{
			$table     = $params['table'] ?? CLI::getOption('table') ?? 'hkm_sessions';
	...
	protected function prepare(string $class): string
	{
		...
		if ($this->getOption('session'))
		{
			...
			$data['session'] = true;
			$data['table']   = is_string($table) ? $table : 'hkm_sessions';
			$data['DBGroup'] = is_string($DBGroup) ? $DBGroup : 'default';
			$data['matchIP'] = hkm_config('App')->sessionMatchIP;
		}
	...
	{
		return gmdate(hkm_config('Migrations')::$timestampFormat) . basename($filename);
	}
}
