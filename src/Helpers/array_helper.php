...
/**
 * Hkm_code Array Helpers
 */

if (! function_exists('hkm_dot_array_search'))
{
	/**
	 ...
	 * @return mixed
	 */
	function hkm_dot_array_search(string $index, array $array)
	{
		...
		return hkm_array_search_dot($segments, $array);
	}
}

if (! function_exists('hkm_array_search_dot'))
{
	/**
	 * Used by `hkm_dot_array_search` to recursively search the
	 ...
	 * @return mixed
	 */
	function hkm_array_search_dot(array $indexes, array $array)
	{
		...
		if ($currentIndex === '*')
		{
			...
			{
				$answer[] = hkm_array_search_dot($indexes, $value);
			}
		...
		{
			return hkm_array_search_dot($indexes, $array[$currentIndex]);
		}
...
if (! function_exists('hkm_array_deep_search'))
{
	/**
	 ...
	 * @return mixed|null
	 */
	function hkm_array_deep_search($key, array $array)
	{
		if (isset($array[$key]))
		{
			return $array[$key];
		...
		foreach ($array as $value)
		{
			if (is_array($value) && ($result = hkm_array_deep_search($key, $value)))
			{
				return $result;
			}
		}
...
if (! function_exists('hkm_array_sort_by_multiple_keys'))
{
	/**
	 ...
	 *
	 * Example:
	 *     hkm_array_sort_by_multiple_keys($players, [
	 ...
	 * @return boolean
	 */
	function hkm_array_sort_by_multiple_keys(array &$array, array $sortColumns): bool
	{
...
if (! function_exists('hkm_array_flatten_with_dots'))
{
	/**
	 ...
	 */
	function hkm_array_flatten_with_dots(iterable $array, string $id = ''): array
	{
		$flattened = [];
		...
		foreach ($array as $key => $value)
		{
			...
			if (is_array($value))
			{
				$flattened = array_merge($flattened, hkm_array_flatten_with_dots($value, $newKey . '.'));
			}
			else
			{
