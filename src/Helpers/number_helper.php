...
if (! function_exists('number_to_size'))
{
	 ...
	 */
	function number_to_size($num, int $precision = 1, string $locale = "en")
	{
		...
		if ($num >= 1000000000000)
		{
			...
			$unit = hkm_lang('Number.terabyteAbbr', [], $generalLocale);
		}
		elseif ($num >= 1000000000)
		{
			$num  = round($num / 1073741824, $precision);
			$unit = hkm_lang('Number.gigabyteAbbr', [], $generalLocale);
		}
		elseif ($num >= 1000000)
		{
			$num  = round($num / 1048576, $precision);
			$unit = hkm_lang('Number.megabyteAbbr', [], $generalLocale);
		}
		elseif ($num >= 1000)
		{
			$num  = round($num / 1024, $precision);
			$unit = hkm_lang('Number.kilobyteAbbr', [], $generalLocale);
		}
		else
		{
			$unit = hkm_lang('Number.bytes', [], $generalLocale);
		}
...
if (! function_exists('number_to_amount'))
{
	...
	function number_to_amount($num, int $precision = 0, string $locale = null)
	{
		...
		{
			$suffix = hkm_lang('Number.quadrillion', [], $generalLocale);
			$num    = round(($num / 1000000000000000), $precision);
		}
		elseif ($num > 1000000000000)
		{
			$suffix = hkm_lang('Number.trillion', [], $generalLocale);
			$num    = round(($num / 1000000000000), $precision);
		}
		elseif ($num > 1000000000)
		{
			$suffix = hkm_lang('Number.billion', [], $generalLocale);
			$num    = round(($num / 1000000000), $precision);
		}
		elseif ($num > 1000000)
		{
			$suffix = hkm_lang('Number.million', [], $generalLocale);
			$num    = round(($num / 1000000), $precision);
		}
		elseif ($num > 1000)
		{
			$suffix = hkm_lang('Number.thousand', [], $generalLocale);
...
if (! function_exists('format_number'))
{
	...
	function format_number(float $num, int $precision = 1, string $locale = null, array $options = []): string
	{
		...
		$locale = $locale ?? ServicesSystem::REQUEST()::GET_LOCALE();
