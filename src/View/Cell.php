...
class Cell
{
	...
	public function render(string $library, $params = null, int $ttl = 0, string $cacheName = null): string
	{
		...
		{
			$instance->initController(ServicesSystem::REQUEST(), ServicesSystem::RESPONSE(), ServicesSystem::LOGGER());
		}

		if (! method_exists($instance, $method))
		{
			throw ViewException::FOR_INVALID_CELL_METHOD($class, $method);
		}
		...
		if ($paramCount === 0)
		{
			if (! empty($paramArray))
			{
				throw ViewException::FOR_MISSING_CELL_PARAMETERS($class, $method);
			}
		...
		else
		{
			...
			foreach (array_keys($paramArray) as $key)
			{
				...
				{
					throw ViewException::FOR_INVALID_CELL_PARAMETER($key);
				}
			}
	...
	protected function determineClass(string $library): array
	{
		...
		if (empty($class))
		{
			throw ViewException::FOR_NO_CELL_CLASS();
		}

		if (! class_exists($class, true))
		{
			throw ViewException::FOR_INVALID_CELL_CLASS($class);
		}
