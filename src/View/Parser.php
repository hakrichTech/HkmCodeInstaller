...
use Hkm_code\Vezirion\Settings\View as ViewConfig;
use ParseError;
...
class Parser extends View
{
	...
	public function render(string $view, array $options = null, bool $saveData = null): string
	{
		...
		// Was it cached?
		if (isset($options['cache']) && ($output = hkm_cache($cacheName)))
		{
		...
		if (! is_file($file))
		{
			...
			if (empty($file))
			{
				throw ViewException::FOR_INVALID_FILE($fileOrig);
			}
		}
	...
	protected function parseConditionals(string $template): string
	{
		...
		{
			ob_end_clean();
			throw ViewException::FOR_TAG_SYNTAX_ERROR(str_replace(['?>', '<?php '], '', $template));
		}
