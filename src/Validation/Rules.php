...
use Hkm_code\Vezirion\Settings\Database;
...
class Rules
{
	...
	public function differs(string $str = null, string $field, array $data): bool
	{
		...
		{
			return $str !== hkm_dot_array_search($field, $data);
		}
	...
	public function is_not_unique(string $str = null, string $field, array $data): bool
	{
		...
		$db = hkm_config('Database')::CONNECT($data['DBGroup'] ?? null);
	...
	public function is_unique(string $str = null, string $field, array $data): bool
	{
		...
		$row = $db::TABLE($ignoreField)
				  ->select('1')
				  ->where($field, $str)
				  ->limit(1);

		if (! empty($table) && ! empty($ignoreValue) && ! preg_match('/^\{(\w+)\}$/', $ignoreValue))
		{
			$row = $row->where("{$table} !=", $ignoreValue);
		}
	...
	public function required($str = null): bool
	{
		...
		return is_array($str) ? count($str)>0: (trim($str) !== '');
	   }
	...
	public function required_with($str = null, string $fields = null, array $data = []): bool
	{
		...
		foreach ($fields as $field)
		{
			if ((array_key_exists($field, $data) && ! empty($data[$field])) ||
				(strpos($field, '.') !== false && ! empty(hkm_dot_array_search($field, $data)))                )
			{
