...
class CLI
{
    ...
    public static function prompt(string $field, $options = null, $validation = null): string
	{
		...
		if ($validation)
		{
			...
			while ($i)
			{
				$f = static::validate($field, $input, $validation);
    ...
    protected static function validate(string $field, string $value, $rules): bool
	{
		...
		$label      = $field;
		$tempField      = 'temp';
		$validation = ServicesSystem::VALIDATION(null, false);
		...
		$validation::RUN([$tempField => $value]);

		if ($validation::HAS_ERROR($tempField))
		{
			static::error($validation::GET_ERROR($tempField));
			return true;
		}else{
			return false;
		}
