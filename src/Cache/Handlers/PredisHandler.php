...
use Hkm_code\Vezirion\Settings\Cache;
use Exception;
use Predis\Client;
...
class PredisHandler extends BaseHandler
{
	...
	public function get(string $key)
	{
		...
		$data = array_combine([
			...
			$this->redis->hmget($key, ['__hkm_type', '__hkm_value'])
		);

		if (! isset($data['__hkm_type'], $data['__hkm_value']) || $data['__hkm_value'] === false)
		{
			return null;
		}

		switch ($data['__hkm_type'])
		{
			case 'array':
			case 'object':
				return unserialize($data['__hkm_value']);
			case 'boolean':
			case 'integer':
			case 'double': // Yes, 'double' is returned and NOT 'float'
			case 'string':
			case 'NULL':
				return settype($data['__hkm_value'], $data['__hkm_type']) ? $data['__hkm_value'] : null;
			case 'resource':
			default:
				return null;
		}
	...
	public function save(string $key, $value, int $ttl = 60)
	{
		...
		if (! $this->redis->hmset($key, ['__hkm_type' => $dataType, '__hkm_value' => $value]))
		{
			return false;
		}
	...
	public function getMetaData(string $key)
	{
		...
		$data = array_combine(['__hkm_value'], $this->redis->hmget($key, ['__hkm_value']));

		if (isset($data['__hkm_value']) && $data['__hkm_value'] !== false)
		{
			$time = time();
