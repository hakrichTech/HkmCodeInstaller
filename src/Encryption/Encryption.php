...
use Hkm_Config\Hkm_Bin\Encryption as Hkm_BinEncryption;

/**
 * Hkm_code Encryption Manager
 *
...
class Encryption
{
     ...
     */
    public function __construct(?Hkm_BinEncryption $config = null)
    {
        $config = $config ?? new Hkm_BinEncryption();
     ...
     */
    public function initialize(?Hkm_BinEncryption $config = null)
    {
        if ($config) {
            $this->key    = $config::$key;
            $this->driver = $config::$driver;
        ...
        $handlerName     = 'Hkm_code\\Encryption\\Handlers\\' . $this->driver . 'Handler';
