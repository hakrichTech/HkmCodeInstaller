...
class Entity implements JsonSerializable
{
    ...
    protected function castAs($value, string $attribute, string $method = 'get')
    {
        ...
        $handlers = array_merge(self::$defaultCastHandlers,$this->castHandlers);
    ...
    */
	...
	{
        if(!empty($dataString)) $this->attributes['deleted_at'] = new Time($dateString, 'UTC');
        else $this->attributes['deleted_at'] = '';
		return $this;
    ...
    public function getUpdatedAt(string $format = 'Y-m-d H:i:s')
    {
       ...
       $this->attributes['updates_at'] = $this->mutateDate($this->attributes['uppdated_at']);

       $timezone = $this->timezone ?? 'UTC';

       $this->attributes['uppdated_at']->setTimezone($timezone);

       return $this->attributes['uppdated_at']->format($format);
    }
