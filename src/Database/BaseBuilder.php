...
use Hkm_code\Database\MySQLi\Connection;
...
class BaseBuilder
{
	 ...
	 * COUNT string
	 *
	 * @used-by HKM_DB_driver::count_all()
	...
	public function select($select = '*', bool $escape = null)
	{
		...
		foreach ($select as $val)
		{
			...
			if ($val !== '')
			{
				 ...
				 * @see https://github.com/Hkm_code4/Hkm_code4/issues/1169
				 */
	...
	protected function maxMinAvgSum(string $select = '', string $alias = '', string $type = 'MAX')
	{
		...
		$sql = $type . '(' . $this->db::PROTECT_INDENTIFIERS(trim($select)) . ') AS ' . $this->db::ESCAPE_IDENTIFIERS(trim($alias));
		$this->QBSelect[]   = $sql;
		...
		foreach ((array) $from as $val)
		{
			if (strpos($val, ',') !== false)
			{
				foreach (explode(',', $val) as $v)
				{
					...
					$this->QBFrom[] = $v = $this->db::PROTECT_INDENTIFIERS($v, true, null, false);
				}
			}
			else
			{
				...
				$this->QBFrom[] = $this->db::PROTECT_INDENTIFIERS($val, true, null, false);
			}
		}
	...
	public function join(string $table, string $cond, string $type = '', bool $escape = null)
	{
		...
		else
		{
			...
			foreach ($conditions as $i => $condition)
			{
				...
				$cond .= $joints[$i];
				$cond .= preg_match("/(\(*)?([\[\]\w\.'-]+)" . preg_quote($operator) . '(.*)/i', $condition, $match) ? $match[1] . $this->db::PROTECT_INDENTIFIERS($match[2]) . $operator . $this->db::PROTECT_INDENTIFIERS($match[3]) : $condition;
			}
		}
		...
		if ($escape === true)
		{
			$table = $this->db::PROTECT_INDENTIFIERS($table, true, null, false);
		}
	...
	protected function _whereIn(string $key = null, $values = null, bool $not = false, string $type = 'AND ', bool $escape = null, string $clause = 'QBWhere')
	{
		...
		if ($escape === true)
		{
			$key = $this->db::PROTECT_INDENTIFIERS($key);
		}
	...
	public function set($key, ?string $value = '', bool $escape = null)
	{
		...
		foreach ($key as $k => $v)
		{
			...
			{
				$bind= $this->setBind($k, $v, $escape);
				$this->QBSet[$this->db::PROTECT_INDENTIFIERS($k, false, $escape)] = ":$bind:";
			}
			else
			{
				$this->QBSet[$this->db::PROTECT_INDENTIFIERS($k, false, $escape)] = $v;
			}
		}
	...
	protected function compileFinalQuery(string $sql): string
	{
		...
		{
			$query::SWAP_PREFIX($this->db::$DBPrefix, $this->db::$swapPre);
		}
	...
	public function countAll(bool $reset = true)
	{
		...
		$sql = $this->countString . $this->db::ESCAPE_IDENTIFIERS('numrows') . ' FROM ' .
				$this->db::PROTECT_INDENTIFIERS($table, true, null, false);
	...
	public function countAllResults(bool $reset = true)
	{
		...
		if ($this->QBDistinct === true || ! empty($this->QBGroupBy))
		{
			...
			$sql    = $this->countString . $this->db::PROTECT_INDENTIFIERS('numrows') . "\nFROM (\n" . $this->compileSelect() . "\n) HKM_count_all_results";
			// Restore SELECT part
		...
		{
			$sql = $this->compileSelect($this->countString . $this->db::PROTECT_INDENTIFIERS('numrows'));
		}
	...
	public function insertBatch(array $set = null, bool $escape = null, int $batchSize = 100)
	{
		...
		{
			$sql = $this->_insertBatch($this->db::PROTECT_INDENTIFIERS($table, true, $escape, false), $this->QBKeys, array_slice($this->QBSet, $i, $batchSize));
	...
	{
		$in = 'INSERT ' . $this->compileIgnore('insert') . 'INTO ' . $table . ' (' . implode(', ', $keys) . ')';
		return str_replace(['"',"'"],'',$in).' VALUES ' . implode(', ', $values);
	}
	...
	public function setInsertBatch($key, string $value = '', bool $escape = null)
	{
		...
		foreach ($keys as $k)
		{
			$this->QBKeys[] = $this->db::PROTECT_INDENTIFIERS($k, false, $escape);
		}
	...
	{
		return 'INSERT ' . $this->compileIgnore('insert') . 'INTO ' . str_replace('"','',$table) . ' (' . str_replace('"','',implode(', ', $keys)) . ') VALUES (' . implode(', ', $unescapedKeys) . ')';
	}
	...
	protected function _update(string $table, array $values): string
	{
		...
		{
			$valStr[] = str_replace('"','',$key) . ' = ' . $val;
		}

		return 'UPDATE ' . $this->compileIgnore('update') . str_replace('"','',$table) . ' SET ' . implode(', ', $valStr)
	...
	public function updateBatch(array $set = null, string $index = null, int $batchSize = 100)
	{
		...
		for ($i = 0, $total = count($this->QBSet); $i < $total; $i += $batchSize)
		{
			$sql = $this->_updateBatch($table, array_slice($this->QBSet, $i, $batchSize), $this->db::PROTECT_INDENTIFIERS($index)
			);
	...
	public function setUpdateBatch($key, string $index = '', bool $escape = null)
	{
		...
		foreach ($key as $v)
		{
			...
			foreach ($v as $k2 => $v2)
			{
				...
				$clean[$this->db::PROTECT_INDENTIFIERS($k2, false, $escape)] = ":$bind:";
			}
	...
	{
		$table = $this->db::PROTECT_INDENTIFIERS($this->QBFrom[0], true, null, false);
	...
	{
		$column = $this->db::PROTECT_INDENTIFIERS($column);
	...
	{
		return 'DELETE ' . str_replace('"','',$this->compileIgnore('delete')) . 'FROM ' . str_replace('"','',$table) . $this->compileWhereHaving('QBWhere');
	}
	 ...
	 * @return string
	 */
	public function compileSelect($selectOverride = false): string
	{
		...
		else
		{
			...
			else
			{
				...
				foreach ($this->QBSelect as $key => $val)
				{
					...
					$this->QBSelect[$key] = $this->db::PROTECT_INDENTIFIERS($val, false, $noEscape);
				}
		...
		{
			$sql .= "\nFROM " . str_replace('"','',$this->_fromTables());
		}
		...
		if ($this->QBLimit)
		{
			return str_replace('"','',$this->_limit($sql . "\n"));
		}
	...
	protected function compileWhereHaving(string $qbKey): string
	{
		if (! empty($this->$qbKey))
		{
			foreach ($this->$qbKey as &$qbkey)
			{
				...
				foreach ($conditions as &$condition)
				{
					...
					if (! empty($matches[4]))
					{
						...
						if (strpos($matches[4], ':') === false)
						{
							$matches[4] = $this->db::PROTECT_INDENTIFIERS(trim($matches[4]), false, $protectIdentifiers);
						}

						$matches[4] = ' ' . $matches[4];
					}

					$condition = $matches[1] . $this->db::PROTECT_INDENTIFIERS(trim($matches[2]))
			...
			return str_replace('"','',($qbKey === 'QBHaving' ? "\nHAVING " : "\nWHERE ")
					. implode("\n", $this->$qbKey));
		}
		...
		if (! empty($this->QBGroupBy))
		{
			foreach ($this->QBGroupBy as &$groupBy)
			{
				...
				$groupBy = ($groupBy['escape'] === false || $this->isLiteral($groupBy['field']))
					? $groupBy['field']
					: $this->db::PROTECT_INDENTIFIERS($groupBy['field']);
			}
	...
	protected function compileOrderBy(): string
	{
		if (is_array($this->QBOrderBy) && ! empty($this->QBOrderBy))
		{
			foreach ($this->QBOrderBy as &$orderBy)
			{
				...
				{
					$orderBy['field'] = $this->db::PROTECT_INDENTIFIERS($orderBy['field']);
				}
