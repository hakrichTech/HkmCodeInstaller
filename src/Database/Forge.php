...
class Forge
{
	...
	public function createDatabase(string $dbName, bool $ifNotExists = false): bool
	{
		...
		if ($this->createDatabaseStr === false)
		{
			...
			if ($this->db::$DBDebug)
			{
				hkm_log_message('error','This feature is not available for the database you are using..');
		...
		try
		{
			...
			if (! $this->db::QUERY(sprintf($ifNotExists ? $this->createDatabaseIfStr : $this->createDatabaseStr, $dbName, $this->db::$charset, $this->db::$DBCollat)))
			{
				...
				if ($this->db::$DBDebug)
				{
				  hkm_log_message('error','Unable to create the specified database.');
				}
				return false;
		...
		catch (Throwable $e)
		{
			...
			if ($this->db::$DBDebug)
			{
				hkm_log_message('warning',$e->getMessage());
			}
			return false; // @codeCoverageIgnore
		}
	}
	...
	public function addForeignKey(string $fieldName = '', string $tableName = '', string $tableField = '', string $onUpdate = '', string $onDelete = '')
	{
		...
		{
			throw new DatabaseException(hkm_lang('Database.fieldNotExists', [$fieldName]));
		}
	...
	{
		$sql = sprintf($this->dropConstraintStr, $this->db::ESCAPE_IDENTIFIERS($this->db::$DBPrefix . $table),
			$this->db::ESCAPE_IDENTIFIERS($this->db::$DBPrefix . $foreignName));
	...
	protected function _createTable(string $table, bool $ifNotExists, array $attributes)
	{
		...
		$sql = ($ifNotExists) ? sprintf($this->createTableIfStr, $this->db::ESCAPE_IDENTIFIERS($table))
			: 'CREATE TABLE';
		...
		$columns = str_replace('"','`',implode(',', $columns));
		...
		$sql = sprintf($this->createTableStr . '%s', $sql, $this->db::ESCAPE_IDENTIFIERS($table), $columns,
	...
	public function dropTable(string $tableName, bool $ifExists = false, bool $cascade = false)
	{
		...
		$this->db::DISABLED_FOREIGN_KEY_CHECKS();
	...
	protected function _dropTable(string $table, bool $ifExists, bool $cascade)
	{
		...
		if ($ifExists)
		{
			...
			{
				$sql = sprintf($this->dropTableIfStr, $this->db::ESCAPE_IDENTIFIERS($table));
			}
		}

		return str_replace('"','',$sql . ' ' . $this->db::ESCAPE_IDENTIFIERS($table));
	}
	...
	public function renameTable(string $tableName, string $newTableName)
	{
		...
		$result = $this->db::QUERY(sprintf($this->renameTableStr,
				$this->db::ESCAPE_IDENTIFIERS($this->db::$DBPrefix . $tableName),
				$this->db::ESCAPE_IDENTIFIERS($this->db::$DBPrefix . $newTableName))
		);
	...
	public function setAutoIncrementStartingValue($value)
	{
		
		$this->autoValue = (string) $value;
		if (!is_null($this->table)) {
		  $sqls = $this->_alterAuto($this->table);
		  if (is_array($sqls)) {
			foreach ($sqls as $sql)
			{
				$sql = str_replace(['"',"'"],'',$sql);
	...
	protected function _alterTable(string $alterType, string $table, $fields)
	{
		...
		if ($alterType === 'DROP')
		{
			...
			$fields = array_map(function ($field) {
				return 'DROP COLUMN ' . $this->db::ESCAPE_IDENTIFIERS(trim($field));
			}, $fields);
	...
	protected function _processPrimaryKeys(string $table): string
	{
		...
		if ($this->primaryKeys !== [])
		{
			$sql .= ",\n\tCONSTRAINT " . $this->db::ESCAPE_IDENTIFIERS('pk_' . $table)
					. ' PRIMARY KEY(' . implode(', ', $this->db::ESCAPE_IDENTIFIERS($this->primaryKeys)) . ')';
		}
	...
	protected function _alterAuto(string $table)
	{
		$sqls = [];
		$sqls []='ALTER TABLE '.$this->db::ESCAPE_IDENTIFIERS($table).' AUTO_INCREMENT = '.$this->autoValue.';';
		return $sqls;
	}
	...
	protected function _processForeignKeys(string $table): string
	{
		...
		if ($this->foreignKeys !== [])
		{
			foreach ($this->foreignKeys as $field => $fkey)
			{
				...
				$sql .= ",\n\tCONSTRAINT " . $this->db::ESCAPE_IDENTIFIERS($nameIndex)
					. ' FOREIGN KEY(' . $this->db::ESCAPE_IDENTIFIERS($field) . ') REFERENCES ' . $this->db::ESCAPE_IDENTIFIERS($this->db::$DBPrefix . $fkey['table']) . ' (' . $this->db::ESCAPE_IDENTIFIERS($fkey['field']) . ')';
