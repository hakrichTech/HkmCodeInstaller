...
use Hkm_code\Database\Forge as BaseForge;
...
class Forge extends BaseForge
{
	...
	protected function _createTableAttributes(array $attributes): string
	{
		...
		{
			$sql .= ' COLLATE = ' . $this->db::ESCAPE_STRING($this->db::$DBCollat);
		}
	...
	{
		$extraClause = isset($field['after']) ? ' AFTER ' . $this->db::ESCAPE_IDENTIFIERS($field['after']) : '';
	...
	protected function _processIndexes(string $table): string
	{
		...
		for ($i = 0, $c = count($this->keys); $i < $c; $i ++)
		{
			...
			$sql .= ",\n\t{$unique}KEY " . $this->db::ESCAPE_IDENTIFIERS(implode('_', $this->keys[$i]))
