...
class Database
{
	 ...
	 */
	public static function LOAD(array $params = [], string $alias = '')
	{
		if ($alias === '')
		{
		...
		self::$connections[$alias] = self::INIT_DRIVER($params['DBDriver'], 'Connection', $params);
	 ...
	 * @return object
	 */
	public static function LOAD_FORGE(ConnectionInterface $db): object
	{
		...
		return self::INIT_DRIVER($db::$DBDriver, 'Forge', $db);
	}
	 ...
	 * @return object
	 */
	public static function LOAD_UTILS(ConnectionInterface $db): object
	{
		// Initialize database connection if not exists.
		if (! $db->connID)
		{
		...
		return self::INIT_DRIVER($db::$DBDriver, 'Utils', $db);
	}
	 ...
	 * @throws InvalidArgumentException
	 */
	protected static function PARSE_DSN(array $params): array
	{
	 ...
	 * @return object
	 */
	protected static function INIT_DRIVER(string $driver, string $class, $argument): object
	{
		$class = $driver . '\\' . $class;

		if (strpos($driver, '\\') === false)
		{
			$class = "Hkm_code\Database\\{$class}";
		}
