...
class MigrationRunner
{
	...
	public static function FIND_MIGRATIONS(): array
	{
		...
		$namespaces = self::$namespace ? [self::$namespace] : array_keys(array_merge(HKM_NAMESPACES__['system'],HKM_NAMESPACES__['app']));
		...
		{
			foreach (self::FIND_NAMESPACE_MIGRATIONS($namespace) as $migration)
			{
...
print_r(self::$path);
	...
	public static function GET_HISTORY(string $group = 'default'): array
	{
		...
		$builder = self::$db::TABLE(self::$table);
